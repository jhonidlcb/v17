...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/client/stage-invoices/:stageId/download-resimple", authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      // Título BOLETA a la derecha
      doc.fontSize(22).fillColor('#1e293b').text('BOLETA', pageWidth - 130, yPos, { width: 100 });
      yPos += 15;
      doc.fontSize(9).fillColor('#64748b').text('RÉGIMEN RESIMPLE - SET PARAGUAY', pageWidth - 220, yPos, { width: 190, align: 'right' });
      yPos += 10;
      doc.fontSize(8).fillColor('#64748b').text(project.name, pageWidth - 220, yPos, { width: 190, align: 'right' });
      ...
      doc.rect(leftMargin, yPos, contentWidth, 28).fillColor('#ffffff').fill();
      ...
      doc.text(`$ ${parseFloat(stage[0].amount).toFixed(2)}`, leftMargin + 370, yPos + 10);
      doc.text(`$ ${parseFloat(stage[0].amount).toFixed(2)}`, leftMargin + 465, yPos + 10);

      doc.rect(leftMargin, yPos, contentWidth, 28).strokeColor('#cbd5e1').stroke();
      yPos += 40;
      ...
      doc.rect(totalsX, yPos, totalsBoxWidth, 55).fillColor('#f8fafc').fill();
      doc.rect(totalsX, yPos, totalsBoxWidth, 55).strokeColor('#cbd5e1').stroke();

      doc.fontSize(9).fillColor('#475569');
      doc.text('Subtotal:', totalsX + 12, yPos + 8);
      doc.text(`$ ${parseFloat(stage[0].amount).toFixed(2)}`, totalsX + 130, yPos + 8);
      
      doc.text('IVA (Exento):', totalsX + 12, yPos + 22);
      doc.text('$ 0.00', totalsX + 130, yPos + 22);

      // Total destacado
      doc.rect(totalsX, yPos + 32, totalsBoxWidth, 23).fillColor('#1e293b').fill();
      doc.fontSize(11).fillColor('#ffffff');
      doc.text('TOTAL:', totalsX + 12, yPos + 39);
      doc.text(`$ ${parseFloat(stage[0].amount).toFixed(2)}`, totalsX + 130, yPos + 39);
